...
#
# Async client for PubMed's eUtils API. Searches for research papers that
# reference specific clinical trials (via NCT ID), returning full details.
# Consumed by run_ingestion.py; not intended to be run directly.
#
# DESIGN: Two-step search (esearch for IDs → efetch for details) due to
# PubMed's API structure. Implements rate limiting (400ms between requests)
# to respect PubMed's 3req/sec limit without an API key.
##############################################################################
...
class PubMedClient:
    """
    Async context-manager client for PubMed eUtils API.
    ...
    async def fetch_papers_for_trial(
        ...
        Handles esearch (find IDs) and efetch (get details) internally.
    ...
    async def _search_paper_ids(
        ...
        Returns:
            List of PubMed paper ID strings, or empty list on failure.
        """
    ...
    async def _fetch_paper_details(
        ...
        Processes IDs in batches of FETCH_BATCH_SIZE.
    ...
    async def _fetch_batch(
        ...
        Parses XML response into Python dictionaries.
    ...
    def _extract_paper_fields(self, article_element: Any) -> dict[str, Any] | None:
        ...
        Returns:
            Dictionary with the paper's key fields, or None if extraction failed.
        """
